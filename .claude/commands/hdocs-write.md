...
6. Shift+Esc로 표 탈출
7. ★검증★ screenshot → 커서가 표 바깥이면 ★성공★ / 안 보이면 재촬영 / 표 안이면 ★실패★ → Shift+Esc 재시도 (최대 3회). **검증 통과 전 본문 입력 금지**
8. **Enter×2 (안전 지대 확보)** → 표와 다음 내용 사이 빈 줄 생성. 불필요한 빈 줄은 나중에 삭제
...
문서 작성 중 아래 시점에서 반드시 검증:

| 게이트 | 시점 | 검증 항목 |
|--------|------|----------|
| G1 | 스타일 설정 직후 | F6 매핑 확인, Ctrl+숫자 테스트 → 스타일 적용 확인 → Ctrl+Z |
| G2 | 각 섹션 완료 시 | screenshot으로 정렬/서식 시각 확인 |
| G3 ⛔ | 표 생성 직후 | Shift+Esc → ★검증★ → Enter×2 (안전 지대) |
| G4 ⛔ | 표 이후 본문 재개 시 | 첫 문단 입력 후 screenshot → 표 밖인지 확인 |
| G5 | 페이지 전환 시 | 글자수 확인 + screenshot |
| G6 | 문서 완료 시 | 전체 페이지 순회 screenshot + 저장 |

**G3, G4는 FATAL 등급** — 스킵 시 내용 유실 위험
