...
| 금지 | 결과 | 안전한 대안 |
|------|------|-----------|
| 다이얼로그에서 `Ctrl+A` | 본문 전체 선택→삭제 | `Home`→`Shift+End` |
| `Page_Down`/`Page_Up` (언더스코어) | 텍스트로 입력됨 | `PageDown`/`PageUp` (붙여쓰기) |
| 다이얼로그에서 `triple_click`→`type()` | 기존값에 추가됨 | `Home`→`Shift+End`→`type()` |
| 표 안 `Ctrl+End`→`Enter`→표 만들기 | 표 안에 표 생성 | `Shift+Esc`→검증→이동 |
| 표 탈출 후 안전 지대 없이 바로 입력 | 본문이 표 셀에 유입→유실 | `Shift+Esc`→검증→`Enter×2`→입력 |

### DANGER — 상태 깨짐
| 금지 | 안전한 대안 |
|------|-----------|
| JS로 scrollTop/canvas 속성 변경 | MCP 도구만 사용 |
| `F5`, `Ctrl+L/T/W/N` | 금지 (브라우저 조작) |

### CAUTION — 기능 실패
| 주의 | 올바른 방법 |
|------|-----------|
| F6 "설정..." 버튼에 find()/좌표 | JS TreeWalker (D절 코드) |
| 머리말/꼬리말에서 Escape | 본문 영역 더블클릭 |
| ref ID 하드코딩 | find()/read_page 동적 탐색 |
...
### 3단계: 문단부호 활성화 ★ 필수 / 격자 보기 ★ 선택
- **문단부호**: 반드시 활성화 (보기 메뉴 → 표시/숨기기 → 문단 부호)
  - 줄바꿈(↵), 빈 줄, 문단 경계 시각적 표시 → 좌표 클릭 정확도 직접 영향
- **격자 보기**: 기본 비활성화 권장
  - 표가 있는 문서에서 격자 점선이 표 테두리와 혼동됨
  - 도형/개체 정밀 배치가 필요한 경우에만 일시적으로 활성화
...
항상 동적으로 탐색:
```
find(query="대상") ─[찾음]→ ref로 조작
                   └─[못찾음]→ read_page(filter="interactive") ─[찾음]→ 조작
                                                               └─[못찾음]→ screenshot+zoom 좌표 파악
```
...
| DO | DON'T |
|----|-------|
| 필드 클릭→`Home`→`Shift+End`→`type("값")` | `Ctrl+A` (본문 전체 선택됨!) |
| `find`→`form_input` 또는 위 방법 | `triple_click`→`type()` (추가됨, 대체 아님) |
...
```
**활용**: 쪽 번호, 현재 스타일. 실패 시 screenshot으로 확인.

**⚠ 상태바 모드 구분 — 신뢰도 제한:**
| 상태 | status_page 패턴 | 신뢰도 |
|------|-----------------|--------|
| 본문 | "N글자" (슬래시 없음) | 중 (셀일 수도 있음) |
| 셀 편집 (확실) | "N/M글자" (슬래시 있음) | **높음** |

- 슬래시(`/`) 포함 = **확실히 셀 편집** → 신뢰 가능
- 슬래시 없음 = **본문 또는 셀** → 상태바 단독 판정 불가
- 확실한 확인: screenshot, 또는 Tab 키 동작 테스트 (Tab→셀 이동이면 표 안)

**상태 확인 의사결정:**
```
JS DOM 코드 실행 ─[값 있음]→ 사용
                └─[null/빈값]→ screenshot ─[확인 가능]→ 사용
                                          └─[불명확]→ zoom(region=[해당 영역])
```
...
| DO | DON'T |
|----|-------|
| `PageDown`/`PageUp` (붙여쓰기) | `Page_Down`/`Page_Up` (텍스트 입력됨!) |
| HWP 단축키 (Ctrl+F, F6, F7) | `F5`/`Ctrl+L`/`Ctrl+T`/`Ctrl+W`/`Ctrl+N` |
...
- 페이지 단위: `PageDown` / `PageUp` (**⚠ 반드시 붙여쓰기! `Page_Down`/`Page_Up` 언더스코어 형태는 텍스트로 입력됨**)

**페이지/뷰포트 이동 (커서 이동 아님):**
- 페이지 단위: `key("PageDown")` / `key("PageUp")` (커서도 함께 이동)
...
| 키 | 이유 |
|----|------|
| `Page_Down` / `Page_Up` | **텍스트로 입력됨** (문서 훼손!) → `PageDown`/`PageUp` 사용 |
| `F5` | 브라우저 새로고침 |
...
| DO | DON'T |
|----|-------|
| JS TreeWalker로 "설정..." 클릭 (D절) | find()/좌표로 "설정..." 클릭 |
| 스타일이 생성하는 기호 신뢰 | type()에 ○/- 등 기호 수동 포함 |
| 개요 스타일 사용 시 내용만 입력 | 번호 수동 입력 (이중 번호 발생) |
...
**⚠ 문단 번호 리셋 (HWP 고유 동작):**
같은 문단 번호 스타일이 **연속된 문단에서만** 번호가 증가한다 (1→2→3).
중간에 다른 스타일의 문단이 끼면 번호가 **1로 리셋**된다.
```
개요 2 → "가."  ← 1번째
개요 2 → "나."  ← 연속 → 정상 증가
바탕글 → (본문)
개요 2 → "가."  ← 리셋! "다."가 아님
```
번호를 연속 배치할 수 있으면 같은 스타일을 끊지 않고 사용한다.
번호를 끊어서 작성해야 하면 문단 번호 스타일 대신 **번호 없는 스타일 + 번호 직접 입력**으로 처리한다.
...
**워크플로우: 스타일 생성/편집 시 기호가 필요하면 D절 "설정..." 버튼의 글머리표/문단번호에서 기호를 반드시 선택한다. 이 단계를 건너뛰면 기호가 자동 생성되지 않는다.**
...
**⚠ type()에는 순수 내용만 입력. 글머리표/문단번호 기호(○, -, · 등)는 스타일이 자동 생성하므로 텍스트에 포함 금지.**
...
**⚠ 위 □, ○, -, ·, 가. 등은 스타일이 자동으로 붙이는 기호. type()에 기호를 포함하면 이중 출력된다. 텍스트에는 내용만 입력.**
...
- **⚠ 반드시 3회 연타 ★ 필수**: HWP 웹에서 1회 전송 시 스타일 미적용 빈도 높음 (실측 1회 성공률 11%, 3회 약 70%, 5회 95%+). 기본 패턴: `key("ctrl+N ctrl+N ctrl+N")`. 검증 실패 시 추가 2회 → 재검증 → 실패 시 F6 대체
- 10번째=Ctrl+0, 11번째 이후는 접근 불가 → F6 또는 드롭다운 사용

**★검증★ 스타일 적용 확인 (JS → zoom 2단계):**
```
1차: JS → document.querySelector('.e_style_item.on')?.textContent
  ├─ 기대 스타일명 일치 → ★성공★
  ├─ 불일치 → 추가 연타 2회 → JS 재확인
  └─ null/undefined → zoom(region=[스타일 드롭다운 영역]) → 시각적 확인
                       ├─ 일치 → ★성공★
                       └─ 불일치 → 추가 연타 → 최종 실패 시 F6 대체
```
...
4. [글머리표/문단번호]: "설정..." 세 번째 버튼 JS 클릭 → 기호 선택 → 확인 → F6 복귀 ★ 기호 필요한 스타일은 필수!
...
| DO | DON'T |
|----|-------|
| `Shift+Esc` → screenshot 검증 → 이동 | 검증 없이 본문 입력 |
| 표 밖에서 `Ctrl+End` (문서 끝) | 표 안에서 `Ctrl+End` (마지막 셀로 감!) |
| 각 셀마다 `Ctrl+B` 개별 적용 | 볼드 후 Tab (전파 안 됨) |
...
**표 탈출 ★ (Shift+Esc) — ★검증★ + 안전 지대 필수:**
```
key("shift+Escape") → screenshot
  ├─ 커서가 표 바깥 → ★성공★ → Enter×2 (안전 지대) → 본문 입력 시작
  ├─ 커서 안 보임 → screenshot 재촬영 (커서 깜빡임)
  └─ 커서가 표 안 → ★실패★ → Shift+Esc 재시도 (최대 3회)
⚠ 검증 통과 전 본문 입력 금지
⚠ Enter×2 없이 바로 type() → 표 마지막 셀에 입력될 위험 (내용 유실, 복구 불가)
```
...
```
★검증★ JS `status_page` → 슬래시 포함이면 셀 진입 성공 / 아니면 screenshot으로 재확인
...
셀 블록 선택(F5/드래그)은 자동화가 불가능하다:
- **F5 키**: 브라우저가 새로고침으로 가로채 사용 불가
- **CDP 드래그**: 좌표 정밀도 한계로 실패율이 매우 높음

**원칙: 셀 블록이 필요한 기능은 대체 경로를 우선 사용하고, 대체 불가능한 경우에만 사용자에게 수동 블록을 안내한다.**

#### 대체 경로 (★ 셀 블록 없이 자동화 가능)

| 기능 | 대체 경로 |
|------|----------|
| 셀 테두리/배경 | 표 메뉴 → 셀 테두리/배경 → **각 셀마다 적용** (셀별 반복) |
| 셀 나누기 | 표 메뉴 → 셀 나누기 (커서 위치 셀) |
| 블록 계산식 | 에이전트가 셀 값을 읽고 계산하여 결과를 대상 셀에 직접 입력 |

**⚠ 대체 불가 → 사용자에게 수동 안내:**

| 기능 | 사용자 안내 |
|------|-----------|
| 셀 합치기 (M) | "합칠 셀을 마우스로 드래그 선택 후 M키를 눌러주세요" |
| 셀 높이를 같게 (H) | "균등 적용할 셀을 마우스로 드래그 선택 후 H키를 눌러주세요" |
| 셀 너비를 같게 (W) | "균등 적용할 셀을 마우스로 드래그 선택 후 W키를 눌러주세요" |
| Ctrl+방향키 크기 조정 | "조정할 셀을 마우스로 드래그 선택 후 Ctrl+방향키를 눌러주세요" |

**⚠ 표/셀 속성**: 표 메뉴로 열리지만 **"셀" 탭 미표시** (셀 블록 없이 열면 기본/여백캡션/테두리/배경/표만 표시). 셀 크기(너비/높이) 개별 설정은 불가.

**설계 원칙**: 셀 합치기/높이너비 균등이 필요 없도록 처음부터 올바른 표 구조로 설계한다.
표 구조 변경이 큰 경우 표 삭제 후 올바른 구조로 재생성이 더 효율적이다.

### F. 셀 나누기

커서 위치만으로 실행 가능 (셀 블록 불필요):
```
1. 나눌 셀에 커서 이동
2. 표 메뉴 → 셀 나누기
...
**⚠ 셀 크기 개별 설정 (표/셀 속성 "셀" 탭)**: 셀 블록 없이 열면 "셀" 탭 미표시 → 자동화 불가. 사용자에게 수동 블록 안내 (E절 참조).

**글자 크기에 맞춘 높이 자동 조정:**
표 만들기 시 높이="자동" → 셀 내 글자 크기에 따라 행 높이 자동 확장

**셀 높이/너비 균등**: 셀 블록 필수 → 자동화 불가. 사용자에게 수동 블록 안내 (E절 참조).
...
**⚠ "셀" 탭**: 셀 블록 모드에서만 표시됨 (자동화 불가). 셀 크기·안여백·셀보호·제목셀·세로정렬 설정이 필요하면 사용자에게 수동 블록 안내 (E절 참조).

### I. 테두리/배경 설정

1. 셀에 커서 위치
2. 표 메뉴 → 셀 테두리/배경 → **각 셀마다 적용** (셀 블록 없이 사용 가능)
3. 테두리: 선 스타일/굵기/색상, 적용 범위(바깥/안쪽/각각)
4. 배경: 배경색/그라데이션/그림
5. 확인
- **⚠ 여러 셀에 동일 설정**: 셀마다 반복 적용 필요 (셀 블록 "하나의 셀처럼 적용"은 자동화 불가)

### J. 블록 계산식

블록 계산식은 셀 블록이 필수이나 자동화 불가. 대안:
1. **에이전트 계산 (권장)**: screenshot/zoom으로 셀 값을 읽고 합계/평균/곱을 계산하여 결과를 대상 셀에 직접 입력 (get_page_text는 캔버스 렌더링이라 사용 불가)
2. **사용자 수동 블록**: "계산할 셀 범위를 마우스로 드래그해주세요" → 블록 후 표 메뉴 → 블록 계산식
...
**배경색**: 표 메뉴 → 셀 테두리/배경 → 각 셀마다 적용으로 셀별 설정 가능. 또는 볼드+가운데만으로 헤더 구분.
...
**방법 A: 표/셀 속성** (정밀): 셀 블록 필수 (셀 탭은 블록 모드에서만 표시) → 자동화 불가 시 사용자 수동 안내
...
| `P` | 표/셀 속성 | 표 메뉴로 대체 (단, 셀 탭 미표시) |
| `S` | 셀 나누기 | 표 메뉴로 대체 |
| `M` | 셀 합치기 | **수동 블록 필요** |
| `W` / `H` | 너비/높이 균등 | **수동 블록 필요** |
| `Ctrl+방향키` | 크기 미세 조정 | **수동 블록 필요** |

### P. 표 내용 유실 방지 ⛔

**표 작업 전후 반드시 확인:**
- 표 생성 전: 커서가 본문인지 확인 (표 안이 아닌지), 표 위 캡션 입력 완료
- 데이터 입력 후: `Shift+Esc` → screenshot 검증 → `Enter×2` (안전 지대)
- 표 이후 첫 본문 문단 입력 후: screenshot 재검증 (표 밖인지 확인)
- 글자수 급변 감지 시: 즉시 `Ctrl+Z` → 상태 확인 (표 안 입력 의심)

**★ 위반 시**: 표 셀 안에 본문이 유입되면 표 삭제 시 내용도 함께 삭제 → 복구 불가
...
- ★검증★ 스타일 드롭다운이 "바탕글"/개요 → ★성공★ / "머리말"/"꼬리말"/"각주" → ★실패★ → 본문 중앙부 더블클릭 재시도
...
**★검증★ 다이얼로그 열림 확인:**
```
단축키 전송 → read_page 또는 find(query="확인")
  ├─ 다이얼로그 요소 발견 → ★성공★ → 진행
  └─ 미발견 → 에디터 캔버스 클릭 → 포커스 확보 → 재시도 (최대 2회)
```
...
### Undo 체인 보호
- **Ctrl+Z는 1회씩 신중하게**: 연속 연타(`ctrl+z ctrl+z ctrl+z`) 금지 → 과도한 원복 초래
- 1회 undo → JS로 글자수 확인 → 필요시 추가 1회 → 반복
- 과도하게 undo한 경우: `Ctrl+Shift+Z`로 redo (역시 1회씩)
- 글자수 모니터링: `document.querySelector('[data-name="status_page"]')?.textContent`

### 오류 복구 의사결정
```
조작 실패 → 에디터 캔버스 클릭(포커스) → 재시도
         └─ 실패 → Escape(잔여 다이얼로그 제거) → 재시도
                  └─ 실패 → navigate(같은 URL)로 새로고침 → 문단부호 재활성화
```
...
**⛔ 치명적 규칙 섹션 참조** (문서 최상단)
- **한글 IME 고려**: 한글 입력 후 **키보드 단축키** 전에만 짧은 대기
- **단계별 확인**: 복잡한 작업은 각 단계 완료 확인 후 다음 진행
