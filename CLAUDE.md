...
4. **실행 계획 수립**: 최종 산출물을 구체적으로 기획 (문서 구조, 내용 초안, 스타일/서식 등을 마크다운으로 설계) → 작업 항목과 필요한 skill 식별 → TODO 목록으로 정리
   - **⚠ skill 체크포인트**: 각 작업 항목에 필요한 skill을 명시하고, 해당 단계 진입 시 반드시 Read로 로드. skill 라우팅 규칙의 트리거 테이블 참조.
5. **실행**: 계획에 따라 순차 수행, 각 단계에서 명시된 skill을 Read로 로드한 후 진행
...
- **요청을 skill에 매핑하지 말 것.** 작업을 단계별로 분해하고, 각 단계에서 필요한 skill을 Read로 로드한다.
