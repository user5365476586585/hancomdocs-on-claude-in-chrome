...
**⚠ 상태바 모드 구분:**
| 상태 | status_page 패턴 | 예시 |
|------|-----------------|------|
| 본문 | "N글자" (슬래시 없음) | "41글자" |
| 셀 편집 | "N글자" (본문과 동일, 구분 불가) | "42글자" |

셀 진입 확인은 상태바로 불가 → type("X") 후 위치 확인 → Ctrl+Z, 또는 screenshot으로 확인.
...
**★검증★ 스타일 적용 확인:**
```
Ctrl+숫자 전송 → 스타일 드롭다운 텍스트 확인
  ├─ 기대 스타일명과 일치 → ★성공★
  └─ 불일치 → Ctrl+숫자 재전송 (에디터 quirk), 2회 실패 시 F6 대체
```
...
**표 탈출 ★ (Shift+Esc) — ★검증★ 필수:**
```
key("shift+Escape") → screenshot
  ├─ 커서가 표 바깥 → ★성공★ → ArrowDown으로 표 아래 이동
  ├─ 커서 안 보임 → screenshot 재촬영 (커서 깜빡임)
  └─ 커서가 표 안 → ★실패★ → Shift+Esc 재시도 (최대 3회)
⚠ 검증 통과 전 본문 입력 금지. 표 아래 최후 수단: Ctrl+End
```
...
조작 실패 → 에디터 캔버스 클릭(포커스) → 재시도
         └─ 실패 → Escape(잔여 다이얼로그 제거) → 재시도
                  └─ 실패 → navigate(같은 URL)로 새로고침 → 문단부호/격자 재활성화
```
