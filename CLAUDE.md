...
셀 블록 선택은 CDP `left_click_drag`로 자동화 가능. **사전 셀 클릭 → 드래그** 2단계가 핵심.

**⚠ 표 외곽 경계선 클릭/드래그 → 표 개체 선택(8핸들) 발생 → 셀 블록 아님!**

#### 셀 블록 선택 알고리즘
```
1. 표가 화면에 보이도록 스크롤 → screenshot 촬영

2. ★ 셀 편집 모드 진입 (필수 선행 조건):
   - 표 내부 아무 셀의 텍스트 중앙을 left_click → 커서 진입
   - ⚠ 이 단계 없이 드래그하면 표 개체 선택(8핸들)이 됨
   - ⚠ 표 외곽 경계선 클릭 금지 → 개체 선택 모드 진입
   - 셀 진입 확인: type("X") → 표시 위치 확인 → Backspace
     (⚠ Ctrl+Z 금지: 빈 셀에서 undo 스택이 얕아 표 생성까지 되돌려 표 소실됨)

3. 드래그 좌표 계산 (screenshot 좌표):
   - 시작점: 블록 범위 첫 셀 내부 (외곽 경계에서 5px+ 안쪽)
   - 끝점: 블록 범위 마지막 셀 중앙, 또는 표 바깥까지 확장 가능
   - 셀 편집 모드에서 드래그가 셀 경계를 넘으면 자동으로 셀 블록 전환
   - ⚠ 시작점이 표 외곽 경계선에 닿으면 개체 선택으로 전환됨
   - ⚠ 사전 클릭 좌표와 드래그 시작 좌표는 달라도 되지만,
     드래그 시작점도 반드시 셀 내부여야 함 (경계 클릭 → 개체 선택)

   [표 전체 블록 필요 시]
   - 시작점: 1행 1열 내부
   - 끝점: 표 우하단 바깥까지 확장 (표 밖으로 넘어가도 전체 블록됨)

4. left_click_drag(start_coordinate=[시작], coordinate=[끝])

5. 검증 (screenshot 필수 + JS 보조):
   - screenshot (필수): 블록된 셀이 회색 음영 + 빨간 동그라미 표시
   - JS (보조): status_page textContent에서 "0/" 패턴 확인
     ("0/41글자" 등 슬래시가 있으면 셀 블록 성공)
   - screenshot 통과가 최종 판정 기준
   - JS 실패(null/패턴 불일치) 시 사용자에게 셀 블록 상태 확인 요청

6. 실패 시:
   - 표 개체 선택(8핸들)이면: Escape → 2단계(셀 클릭)부터 재시작
   - 텍스트 선택이면: 시작점을 셀 가장자리 쪽으로 조정 후 재시도
   - 최대 3회 재시도
```
...
#### 셀 블록 실패 시 대안
1. **Escape → 셀 클릭 → 재드래그** (가장 흔한 실패: 사전 셀 클릭 누락)
2. **좌표 미세 조정 후 재시도** (±5px, 최대 3회)
3. **처음부터 올바른 구조로 설계** → 합치기 불필요하게
4. **사용자에게 수동 조작 안내**: "마우스로 셀 드래그하여 블록 선택 후 M/W/H 키"
5. **표 삭제 후 재생성**: 구조 변경이 클 때
