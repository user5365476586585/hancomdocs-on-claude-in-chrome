...
사용자 요청을 적절한 skill로 매핑:

| 사용자 의도 | skill | 설명 |
|------------|-------|------|
| 파일/폴더 관리, 대시보드, 내 드라이브 | `/hdocs-dashboard` | 대시보드 파일 관리 |
| 에디터 열기, 탭 찾기, 저장, 새 문서 | `/hdocs-navigate` | 에디터 탐색/제어 기반 |
| 텍스트 입력, 글쓰기, 내용 작성 | `/hdocs-navigate` | 텍스트 입력은 navigate에 포함 |
| 스타일 적용 (개요, 본문 등) | `/hdocs-style` | F6 스타일 다이얼로그 |
| 표 만들기, 셀 편집, 테두리 | `/hdocs-table` | 표 관련 모든 작업 |
| 글꼴, 크기, 정렬, 줄간격 | `/hdocs-format` | 글자/문단 서식 |
| 용지 설정, 머리말, 쪽번호 | `/hdocs-page` | 페이지 레이아웃 |
| 그림, 도형, 수식, 차트 삽입 | `/hdocs-insert` | 개체 삽입 |
| 공문서 작성, 문서 자동 생성 | `/hdocs-write` | 통합 워크플로우 |

### 복합 작업 오케스트레이션
사용자가 여러 단계를 포함하는 작업을 요청하면:
1. 작업을 단계별로 분해
2. 각 단계에 적합한 skill 식별
3. 순서대로 실행 (이전 단계 완료 확인 후 다음 단계)
4. 각 단계 실행 결과를 사용자에게 간략히 보고
