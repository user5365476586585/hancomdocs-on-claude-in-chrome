...
탐색/제어, 스타일, 표 기능은 CLAUDE.md에 내장되어 별도 슬래시 커맨드 없이 자동 사용된다.

## 아키텍처

```
CLAUDE.md (오케스트레이터 + 내장 기능)
├── 에디터 식별 + 범용 조작 원칙
├── 탐색/제어/입력 (내장)        ← 텍스트 입력, 저장, 찾기, 단축키
├── 스타일 관리 (내장)           ← F6, Ctrl+숫자, 배치 생성
├── 표 생성/편집 (내장)          ← 표 만들기, 셀 편집, 캡션
├── 서식 원칙 + Skill 라우팅 규칙
│
├── .claude/commands/ (5개 Skill)
...
| 기능 | 상태 | 사유 | 대안 |
|------|------|------|------|
| 셀 블록 선택 | 수동 | F5=브라우저 가로챔, CDP=좌표 정밀도 부족 | 셀 테두리/나누기/계산식은 메뉴 대체, 합치기/균등은 수동 |
| 수식 넣기 버튼 | 수동 | WASM 캔버스 기반 다이얼로그 | 수식 코드 입력까지 자동화, 넣기는 수동 |
